...
func init() {
	err := event.RegisterTextCompactTemplate("io.nats.jetstream.advisory.v1.max_deliver", `{{ .Time | ShortTime }} [JS Max Deliveries] {{ .Stream }} ({{ .Stream }}) > {{ .Consumer }}: {{ .Deliveries }} deliveries`)
	if err != nil {
		panic(err)
	}
