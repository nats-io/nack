...
type StreamTemplateLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.StreamTemplate, err error)
...
// List lists all StreamTemplates in the indexer.
func (s *streamTemplateLister) List(selector labels.Selector) (ret []*v1beta1.StreamTemplate, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.StreamTemplate))
	})
	return ret, err
}
...
type StreamTemplateNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.StreamTemplate, error)
...
// List lists all StreamTemplates in the indexer for a given namespace.
func (s streamTemplateNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.StreamTemplate, err error) {
...
// Get retrieves the StreamTemplate from the indexer for a given namespace and name.
func (s streamTemplateNamespaceLister) Get(name string) (*v1beta1.StreamTemplate, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("streamtemplate"), name)
	}
	return obj.(*v1beta1.StreamTemplate), nil
}
