...
type StreamLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Stream, err error)
...
// List lists all Streams in the indexer.
func (s *streamLister) List(selector labels.Selector) (ret []*v1beta1.Stream, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Stream))
	})
	return ret, err
}
...
type StreamNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Stream, error)
...
// List lists all Streams in the indexer for a given namespace.
func (s streamNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Stream, err error) {
...
// Get retrieves the Stream from the indexer for a given namespace and name.
func (s streamNamespaceLister) Get(name string) (*v1beta1.Stream, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("stream"), name)
	}
	return obj.(*v1beta1.Stream), nil
}
