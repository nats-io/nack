...
type ConsumerLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.Consumer, err error)
...
// List lists all Consumers in the indexer.
func (s *consumerLister) List(selector labels.Selector) (ret []*v1beta1.Consumer, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.Consumer))
	})
	return ret, err
}
...
type ConsumerNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.Consumer, error)
...
// List lists all Consumers in the indexer for a given namespace.
func (s consumerNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.Consumer, err error) {
...
// Get retrieves the Consumer from the indexer for a given namespace and name.
func (s consumerNamespaceLister) Get(name string) (*v1beta1.Consumer, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("consumer"), name)
	}
	return obj.(*v1beta1.Consumer), nil
}
